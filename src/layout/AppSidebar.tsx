...
import { ChevronDownIcon, GridIcon, TableIcon, UserCircleIcon } from "../icons";
...
const AppSidebar: React.FC = () => {
  ...
  const main: NavItem[] = useMemo(
    () => [
      ...
      { icon: <BookOpen />, name: "My Courses", path: "/my-courses" },
            { icon: <BookOpen />, name: "My Submissions", path: "/submissions" },
  ...
  const user: NavItem[] = useMemo(
    () => [
      ...
      { icon: <Shield />, name: "Login & Security", path: "/security" },
       { icon: <Shield />, name: "Refer & Earn", path: "/referral" },
    ],
    []
  );
  ...
  useEffect(() => {
    ...
    ["main", "learning", "support", "user"].forEach((menuType) => {
      const items =
        menuType === "main"
          ? main
          : menuType === "learning"
