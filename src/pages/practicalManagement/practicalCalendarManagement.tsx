...
import { ChevronLeft, ChevronRight, Calendar, MapPin, Users, User, BookOpen, Clock, Tag, Filter, CheckCircle } from 'lucide-react';
...
const MOCK_PRACTICALS: PracticalSession[] = [
  ...
  // Full Slot (Red - Non-clickable)
  { id: 'P2', courseId: 'C102', venueId: 'V2', instructorId: 'I1', startTime: '2025-10-15T14:00:00.000Z', capacity: 10, bookedSeats: 10, students: ['stud-1', 's2', 's3', 's4', 's5', 's6', 's7', 's8', 's9', 's10'] },
  // Almost Full Slot (Orange) - Student NOT booked
  { id: 'P3', courseId: 'C101', venueId: 'V1', instructorId: 'I3', startTime: '2025-10-17T09:00:00.000Z', capacity: 10, bookedSeats: 8, students: ['stud-2', 's3', 's4', 's5', 's6', 's7', 's8', 's9'] },
  // Available Slot (Green) - Student NOT booked
  ...
  // Available Slot (Green) - Student ALREADY booked
...
const CalendarDay: React.FC<CalendarDayProps> = ({ day, currentMonth, sessions, selectedDate, onDayClick }) => {
  ...
  let countClasses = 'h-2 w-2 rounded-full';
...
export default function PracticalBookingCalendar() {
  ...
  const handleBookSlot = useCallback((sessionId: string) => {
    ...
    alert(`Slot booked successfully for Session ${sessionId}!`);
  }, []);
  ...
  const handleUnbookSlot = useCallback((sessionId: string) => {
    ...
    alert(`Slot unbooked successfully for Session ${sessionId}!`);
  }, []);
  ...
  const renderFilters = () => (
    <div className="p-4 border-b bg-white flex flex-wrap gap-4 items-center">
      ...
      <select name="course" value={filters.course} onChange={handleFilterChange} className="p-2 border rounded-lg text-sm flex-1 min-w-[150px]">
        <option value="">All Courses</option>
        {MOCK_COURSES.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
      </select>

      <select name="venue" value={filters.venue} onChange={handleFilterChange} className="p-2 border rounded-lg text-sm flex-1 min-w-[150px]">
        <option value="">All Venues</option>
        {MOCK_VENUES.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
      </select>

      <button onClick={() => setViewMode(viewMode === 'month' ? 'week' : 'month')} className="p-2 border rounded-lg text-sm text-indigo-700 bg-indigo-100 hover:bg-indigo-200 transition-colors">
    ...
    return (
      <div className="p-6 bg-white overflow-y-auto">
        <h3 className="text-xl font-bold text-gray-800 border-b pb-2 mb-4 flex items-center">
          ...
          Sessions for {selectedDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
        </h3>
                ...
                <div key={session.id} className={cardClass}>
                  ...
                  <div className="space-y-1 text-sm text-gray-700">
                    <p className="flex items-center"><BookOpen size={16} className="mr-2 text-indigo-500" />
                      **Course:** {COURSE_MAP[session.courseId]}</p>
                    <p className="flex items-center"><MapPin size={16} className="mr-2 text-red-500" />
                      **Venue:** {VENUE_MAP[session.venueId]} ({MOCK_VENUES.find(v => v.id === session.venueId)?.location})</p>
                    <p className="flex items-center"><User size={16} className="mr-2 text-green-500" />
                      **Instructor:** {INSTRUCTOR_MAP[session.instructorId]}</p>
                    <p className="flex items-center"><Users size={16} className="mr-2 text-yellow-500" />
                      **Available Seats:** {availableSeats} / {session.capacity}</p>
                  </div>

                  <div className="mt-4 flex justify-end">
                    {isBooked ? (
                      <button onClick={() => handleUnbookSlot(session.id)} className={buttonClass}>
                        Cancel Booking
                      </button>
                    ) : (
                      <button onClick={() => handleBookSlot(session.id)} disabled={isFull} className={buttonClass}>
  ...
  return (
    <div className="flex h-screen ">
      <div className="flex flex-col lg:flex-row w-full overflow-hidden bg-white">

        {/* Left Panel: Calendar and Filters */}
        <div className="lg:w-2/3 flex flex-col border-r border-gray-200">
          ...
          <div className="flex justify-between items-center p-4 bg-gray-50 border-b">
            <button onClick={() => handleMonthChange('prev')} className="p-2 rounded-full hover:bg-gray-200">
              <ChevronLeft size={24} />
            </button>
            <h2 className="text-xl font-bold text-gray-800">
              {currentDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}
            </h2>
            <button onClick={() => handleMonthChange('next')} className="p-2 rounded-full hover:bg-gray-200">
        ...
        {/* Right Panel: Practical Slot Details */}
        <div className="lg:w-1/3 flex flex-col bg-gray-50">
          <h1 className="p-4 text-xl font-bold text-gray-800 bg-gray-100 border-b">
            Slot Details
          </h1>
